-- ============================================================
-- Migration: Fix viz/audio percentages in get_friend_extended
-- Version:   20260318002
--
-- BUG: VERSUS sessions produce TWO session_runs rows (one per
-- team). The raw join in run_stats matched both rows for the
-- player's reservation, so every versus visual/audio choice
-- was counted twice vs a COOP session's once. This skewed
-- percentages toward whatever visual/audio versus sessions used.
--
-- Example: 2 COOP (Dark) + 2 Versus (Strobe) should be 50/50.
-- Old result: Strobe 66.7%, Dark 33.3%  (4 vs rows + 2 coop)
-- New result: Strobe 50.0%, Dark 50.0%  (2 + 2, deduped)
--
-- FIX: Apply the same dedup predicate already used for obj_pct,
-- avg_time_sec, and versus_runs to the visual and audio counts:
--   sr.role IS NULL                          -- COOP: all rows count
--   OR sr.team = rp.team                     -- VERSUS: player's team row only
--   OR (rp.team IS NULL AND sr.team = 1)     -- legacy fallback
-- ============================================================
...
LANGUAGE sql SECURITY DEFINER STABLE AS $$
  WITH run_stats AS (
    SELECT
      ...
      -- ── Visuals (deduped: COOP all rows, VERSUS player-team row only) ─────
      -- Denominator is the deduped row count, not COUNT(*), so versus sessions
      -- are not double-weighted relative to COOP sessions.
      ROUND(100.0
        * COUNT(*) FILTER (WHERE sr.visual = 'V'
            AND (sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)))
        / NULLIF(COUNT(*) FILTER (
            WHERE sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)
          ), 0), 1)                                                                    AS viz_std,

      ROUND(100.0
        * COUNT(*) FILTER (WHERE sr.visual = 'C'
            AND (sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)))
        / NULLIF(COUNT(*) FILTER (
            WHERE sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)
          ), 0), 1)                                                                    AS viz_cosmic,
      ...
      ROUND(100.0
        * COUNT(*) FILTER (WHERE sr.visual = 'R'
            AND (sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)))
        / NULLIF(COUNT(*) FILTER (
            WHERE sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)
          ), 0), 1)                                                                    AS viz_rave,
      ...
      ROUND(100.0
        * COUNT(*) FILTER (WHERE sr.visual = 'S'
            AND (sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)))
        / NULLIF(COUNT(*) FILTER (
            WHERE sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)
          ), 0), 1)                                                                    AS viz_strobe,
      ...
      ROUND(100.0
        * COUNT(*) FILTER (WHERE sr.visual = 'B'
            AND (sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)))
        / NULLIF(COUNT(*) FILTER (
            WHERE sr.role IS NULL OR sr.team = rp.team OR (rp.team IS NULL AND sr.team = 1)
          ), 0), 1)                                                                    AS viz_dark,
