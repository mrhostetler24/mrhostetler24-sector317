-- ============================================================
-- Migration: 20260315002_friend_extended_v3
-- Version:   20260315002
--
-- HOW TO RUN:
...
--
-- WHAT THIS DOES:
--   Fixes coop_pct calculation in get_friend_extended.
--
--   Versus sessions store two rows per run in session_runs (one per team).
--   Dividing co-op row count by total row count (120) understates coop_pct.
--   Fix: divide by deduped run count (role IS NULL OR team = 1) = 76,
--   which matches the player's actual run count shown in their profile.
--
--   All other metrics (viz_*, aud_*, obj_pct, avg_time_sec) are unaffected
--   because visual/audio/objective values are identical across both team
--   rows of a versus run, so duplicate rows cancel out in the ratios.
-- ============================================================
...
LANGUAGE sql SECURITY DEFINER STABLE AS $$
  WITH run_stats AS (
    SELECT
      ...
      -- coop_pct: co-op rows / deduped run count (skip 2nd team row of versus runs)
      ROUND(100.0 * COUNT(*) FILTER (WHERE sr.role IS NULL)
        / NULLIF(COUNT(*) FILTER (WHERE sr.role IS NULL OR sr.team = 1), 0), 1)      AS coop_pct,
