...
import {
  ...
  createShiftAlerts, fetchMyShiftAlerts, dismissShiftAlert,
  fetchUserRoles,
} from "./supabase.js";
...
// TODO: delete after testing — AUTH_ICON (no longer used)
// const AUTH_ICON = {google:"G",microsoft:"M"};
// TODO: delete after testing — AUTH_COLOR (no longer used)
...
function SchedulePanel({currentUser,shifts,setShifts,users,isManager,onAlert,tabOverride,onTabOverrideConsumed}){
  ...
  useEffect(()=>{
    fetchMyShiftAlerts(currentUser.id)
      .then(a=>setShiftAlerts(a.filter(a=>a.shift&&(a.shift.open||a.shift.conflicted||!a.shift.staffId))))
      .catch(()=>{});
  },[]); // eslint-disable-line react-hooks/exhaustive-deps
  async function handleDismissAlert(id){
    try{await dismissShiftAlert(id);setShiftAlerts(p=>p.filter(a=>a.id!==id));}catch(e){onAlert('Error dismissing alert: '+e.message);}
  }
  async function handleNotifyStaff(shift){
    const eligible=users.filter(u=>{
      if(!['staff','manager','admin'].includes(u.access))return false;
      if(u.active===false)return false;
      if(u.id===currentUser.id)return false;
      if(shift.role&&u.role!==shift.role)return false;
      return !shifts.some(sh=>sh.staffId===u.id&&sh.date===shift.date&&!(timeToMin(sh.start)>=timeToMin(shift.end)||timeToMin(sh.end)<=timeToMin(shift.start)));
    });
    if(!eligible.length){onAlert('No eligible staff found for this shift.');return;}
    try{
      await createShiftAlerts(shift.id,eligible.map(u=>u.id));
      onAlert(`Alert sent to ${eligible.length} eligible staff member${eligible.length!==1?'s':''}.`);
    }catch(e){onAlert('Error sending alerts: '+e.message);}
  }
  useEffect(()=>{
  ...
  return(
    <div>
      ...
      {shiftAlerts.map(a=>{const s=a.shift;if(!s)return null;return <div key={a.id} className="alert-banner" style={{marginBottom:'.4rem',justifyContent:'space-between',flexWrap:'wrap',gap:'.4rem'}}>
        <span style={{fontSize:'.85rem'}}>📢 Shift available{s.role?<>: <strong>{s.role}</strong></>:''} &nbsp;·&nbsp; {getDayName(s.date)+', '+fmt(s.date)} {fmt12(s.start)}–{fmt12(s.end)}</span>
        <div style={{display:'flex',gap:'.35rem',flexShrink:0}}>
          <button className="btn btn-warn btn-sm" onClick={()=>{setTab('open');setSelectedDay(s.date);handleDismissAlert(a.id);}}>View →</button>
          <button className="btn btn-s btn-sm" onClick={()=>handleDismissAlert(a.id)}>Dismiss</button>
        </div>
      </div>;})}
      <div className="tabs">
      ...
      {tab==="conflict"&&<>
        ...
        {conflicts.map(s=>{const orig=getU(s.staffId);return <div key={s.id} className="shift-card conflict" style={{padding:'.5rem 1rem',flexWrap:'nowrap'}}>
          ...
          <div style={{display:"flex",gap:".4rem",flexShrink:0}}>
            {isManager&&<button className="btn btn-s btn-sm" onClick={()=>handleNotifyStaff(s)}>📢 Notify</button>}
      ...
      {tab==="dashboard"&&<>
        ...
        <div className="stats-grid">
          {(()=>{
            ...
            const coopRuns=dashRuns.filter(r=>runMode(r)==='coop');
            const vsRuns=dashRuns.filter(r=>runMode(r)==='versus');
