        ...
        <div className="stats-grid">
          {(()=>{
            ...
            return <>
              ...
              {w.envCoop&&(()=>{const tv=topViz(coopRuns);const ta=topAud(coopRuns);const envCol={fontSize:'.56rem',letterSpacing:'.08em',textTransform:'uppercase',color:'var(--muted)',marginBottom:'.15rem'};return<div className="stat-card"><div className="stat-lbl">Env — Co-Op <span style={{fontWeight:400,opacity:.6}}>({coopRuns.length} runs)</span></div><div style={{display:'flex',gap:'1rem',margin:'.25rem 0 .1rem'}}><div><div style={envCol}>Visual</div>{tv?<div className="stat-val stat-val-xs" style={{color:vizColor[tv.code]??'var(--accB)',lineHeight:1}}>{tv.name}</div>:<div className="stat-val stat-val-xs" style={{color:'var(--muted)',lineHeight:1}}>—</div>}</div><div><div style={envCol}>Audio</div>{ta?<div className="stat-val stat-val-xs" style={{color:audColor[ta.code]??'var(--accB)',lineHeight:1}}>{ta.name}</div>:<div className="stat-val stat-val-xs" style={{color:'var(--muted)',lineHeight:1}}>—</div>}</div></div><div className="stat-sub">Visual: {vizLine(coopRuns)}</div><div className="stat-sub">Audio: {audLine(coopRuns)}</div></div>;})()}
              {w.envVs&&(()=>{const tv=topViz(vsRuns);const ta=topAud(vsRuns);const envCol={fontSize:'.56rem',letterSpacing:'.08em',textTransform:'uppercase',color:'var(--muted)',marginBottom:'.15rem'};return<div className="stat-card"><div className="stat-lbl">Env — Versus <span style={{fontWeight:400,opacity:.6}}>({vsRuns.length} runs)</span></div><div style={{display:'flex',gap:'1rem',margin:'.25rem 0 .1rem'}}><div><div style={envCol}>Visual</div>{tv?<div className="stat-val stat-val-xs" style={{color:vizColor[tv.code]??'var(--accB)',lineHeight:1}}>{tv.name}</div>:<div className="stat-val stat-val-xs" style={{color:'var(--muted)',lineHeight:1}}>—</div>}</div><div><div style={envCol}>Audio</div>{ta?<div className="stat-val stat-val-xs" style={{color:audColor[ta.code]??'var(--accB)',lineHeight:1}}>{ta.name}</div>:<div className="stat-val stat-val-xs" style={{color:'var(--muted)',lineHeight:1}}>—</div>}</div></div><div className="stat-sub">Visual: {vizLine(vsRuns)}</div><div className="stat-sub">Audio: {audLine(vsRuns)}</div></div>;})()}
