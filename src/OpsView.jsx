                  ...
                  <div key={res.id} style={{background:"var(--bg2)",border:"1px solid var(--bdr)",borderRadius:8,marginBottom:".6rem",overflow:"hidden"}}>
                    ...
                    <div style={{borderTop:"1px solid var(--bdr)",padding:".65rem 1rem"}}>
                      {rt?.mode==="versus"?(()=>{
                        ...
                        const switchT=pid=>{const newTeam=getTeam(pid)===1?2:1;setVersusTeams(prev=>({...prev,[res.id]:{...(prev[res.id]||{}),[pid]:newTeam}}));setReservations(prev=>prev.map(r=>r.id===res.id?{...r,players:(r.players||[]).map(p=>p.id===pid?{...p,team:newTeam}:p)}:r));updateReservationPlayer(pid,{team:newTeam}).catch(e=>console.error('switchT failed:',e));};
                        ...
                        const slotReses=reservations.filter(r=>r.date===res.date&&r.startTime===res.startTime&&r.status!=='cancelled');
                        const slotMap=calcResVsTeams(slotReses);
                        let sT1=0,sT2=0;slotReses.forEach(r=>{const n=(r.players||[]).length;if(slotMap[r.id]===1)sT1+=n;else sT2+=n;});
                        const slotImbalanced=sT1+sT2>=4&&Math.abs(sT1-sT2)>2;
                        return(<div>{slotImbalanced&&<div style={{background:"rgba(245,158,11,.08)",border:"1px solid rgba(245,158,11,.35)",borderRadius:6,padding:".5rem .75rem",marginBottom:".6rem",fontSize:".82rem",color:"var(--warnL)"}}>⚠ Teams are unbalanced for this slot (Team 1: {sT1}, Team 2: {sT2}). Use the ↓T2 / ↑T1 buttons to rearrange.</div>}<div style={{fontWeight:600,fontSize:".78rem",color:"var(--muted)",marginBottom:".5rem",textTransform:"uppercase",letterSpacing:".05em"}}>Players <span style={{textTransform:"none",fontWeight:400,color:players.length>=maxForRes?"var(--danger)":"var(--muted)"}}>{players.length}/{maxForRes}</span></div><div style={{display:"flex",flexDirection:"column",gap:".65rem"}}>{[1,2].map(tn=>{const tPlayers=tn===1?t1:t2;return(<div key={tn} style={{background:"var(--bg)",border:"1px solid var(--bdr)",borderRadius:6,padding:".5rem .6rem"}}><div style={{fontWeight:700,fontSize:".73rem",color:"var(--muted)",textTransform:"uppercase",letterSpacing:".04em",marginBottom:".35rem"}}>Team {tn} <span style={{fontWeight:400}}>({tPlayers.length}/6)</span></div>{tPlayers.length===0&&<div style={{fontSize:".78rem",color:"var(--muted)",padding:".2rem 0"}}>—</div>}{tPlayers.map(p=>pRow(p,tn))}{addPanel(tn)}</div>);})}</div></div>);
                      })():(
                        <>
