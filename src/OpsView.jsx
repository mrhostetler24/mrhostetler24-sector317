                ...
                return(
                  <div key={res.id} style={{background:"var(--bg2)",border:"1px solid var(--bdr)",borderRadius:8,marginBottom:".6rem",overflow:"hidden"}}>
                    ...
                    {res.status!=="sent"&&res.status!=="completed"&&(
                      <div style={{display:"flex",gap:".5rem",flexWrap:"wrap",padding:".3rem 1rem .6rem",alignItems:"center"}}>
        ...
        return(
          <div className="mo"><div className="mc">
            {wiStep==="details"&&<>
              ...
              <div className="f"><label>Type</label><select value={wi.typeId} onChange={e=>setWi(p=>({...p,typeId:e.target.value,splitA:0,extraSlots:[],playerCount:1}))}><option value="">— Select —</option>{filteredResTypes.map(rt=><option key={rt.id} value={rt.id}>{rt.name}</option>)}</select>{wiTime&&filteredResTypes.length===0&&<div style={{fontSize:".78rem",color:"var(--danger)",marginTop:".3rem"}}>No booking types available for this time slot.</div>}</div>
              ...
              {!wiIsPriv&&<div className="f"><label>Player Count</label><input type="number" min={1} max={wiCap?wiCap.total:20} value={wi.playerCount} onChange={e=>setWi(p=>({...p,playerCount:Math.max(1,Math.min(wiCap?wiCap.total:99,+e.target.value)),splitA:0,extraSlots:[]}))} disabled={!wi.typeId}/>{!wi.typeId&&<div style={{fontSize:".75rem",color:"var(--muted)",marginTop:".2rem"}}>Select a type first.</div>}{wi.typeId&&wiCap&&wiTime&&<div style={{fontSize:".75rem",color:wiCap.total<3?"var(--warn)":"var(--muted)",marginTop:".2rem"}}>{wiCap.total} spot{wiCap.total!==1?"s":""} available in this slot</div>}</div>}
