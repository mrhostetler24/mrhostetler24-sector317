...
function BookingWizard({resTypes,sessionTemplates,reservations,allReservations,currentUser,users,activeWaiverDoc,onBook,onPayCreate,onFinalize,onClose}){
  ...
  return(
    <div className="mo"><div className="mc" style={{maxWidth:660}}>
      ...
      {selMode&&step>1&&<div style={{fontSize:".82rem",color:"var(--muted)",marginBottom:".9rem",padding:".45rem .75rem",background:"var(--surf2)",border:"1px solid var(--bdr)",borderRadius:5,lineHeight:1.7}}>
        {'Booking a '}
        <span style={{color:"var(--acc)",fontWeight:700}}>{selMode==="coop"?"Co-Op":"Versus"}</span>
        {' match'}
        {selStyle&&<>{' for '}{selStyle==="open"?"an":"a"}{' '}<span style={{color:"var(--acc)",fontWeight:700}}>{selStyle==="open"?"Open Play":"Private Team"}</span>{' group'}</>}
        {selDate&&<>{' on '}<span style={{color:"var(--acc)",fontWeight:700}}>{new Date(selDate+"T12:00:00").toLocaleDateString("en-US",{weekday:"short",month:"short",day:"numeric",year:"numeric"})}</span></>}
        {selSlots.length>0&&<>{' at '}<span style={{color:"var(--acc)",fontWeight:700}}>{[...new Set(selSlots.map(s=>s.startTime))].sort().map(t=>fmt12(t)).join(", ")}</span></>}
      </div>}
        ...
        return <>
          ...
          {(selSlots.length===0||addingMore)&&<>
            ...
            <div className="slot-grid">{slotStatuses.map(({tmpl,status,altStatus,added})=>{
              ...
              const renderLanes=(lanes,forMode)=>lanes?.length>0?<div style={{marginTop:".35rem",display:"flex",flexDirection:"column",gap:"2px"}}>{lanes.map(l=>{const cap=laneCapacity(l.mode||"coop");const isFull=l.type!==null&&(l.type==="private"||l.playerCount>=cap);const label=l.type===null?"Free":l.type==="private"||l.playerCount>=cap?"Full":l.mode==="coop"?`Co-Op ${l.playerCount}/6`:`Versus ${l.playerCount}/12`;const col=l.type===null?"rgba(200,224,58,.5)":isFull?"var(--muted)":l.mode===forMode?"var(--okB)":"var(--warn)";return<div key={l.laneNum} style={{fontSize:".6rem",fontFamily:"var(--fd)",letterSpacing:".04em",display:"flex",justifyContent:"space-between",color:col}}><span>Lane {l.laneNum}</span><span>{label}</span></div>;})}</div>:null;
      ...
      <div className="ma">
        ...
        {step<steps.length
          ?<>{!paymentSuccess&&step===5
              ...
              :<button className="btn btn-p" disabled={!canNext[step]} onClick={()=>startTransition(()=>setStep(s=>s+1))}>Continue →</button>
            }</>
