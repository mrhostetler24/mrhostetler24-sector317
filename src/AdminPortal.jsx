    ...
    <div className="content">
      ...
      {tab==="dashboard"&&<>
        ...
        <div className="stats-grid">
          {(()=>{
            ...
            return <>
              ...
              {w.avgRunTime&&<div className="stat-card"><div className="stat-lbl">Avg Run Time</div><div className="stat-val">{fmtSec(avgSec([...coopRuns,...vsRuns]))}</div><div className="stat-sub">Co-Op {fmtSec(avgSec(coopRuns))} · Versus {fmtSec(avgSec(vsRuns))}</div><div className="stat-sub">Full timer: Co-Op {fullTimerPct(coopRuns)??'—'}% · Versus {fullTimerPct(vsRuns)??'—'}%</div></div>}
            </>;
          })()}
        </div>
        ...
        {(()=>{
          ...
          const timelineMinPx=Math.max(1200,Math.ceil(tlSpan/60*350));
